...
啟動方式：
    python -m streamlit run dashboard/app_v5_2.py
...
st.sidebar.title("❤️ HeartHealth Hub")
...
# =========================================================
...
elif page == "資料中心｜資料庫與品質":
    st.title("資料中心｜資料清理、SQLite 資料庫與資料品質")
    ...
    if not group_risk_df.empty:
        group_display_df = group_risk_df.rename(columns={
            ...
            "group": "分組",
            "count": "分組總筆數",
            "risk_count": "風險紀錄數（target=1）",
            "risk_rate": "風險紀錄比例（%）",
        })
        plot_bar(group_display_df, "分組", "風險紀錄比例（%）", "年齡 / 血壓 / 膽固醇分組風險紀錄比例", color="分組類型")
...
# =========================================================
    ...
    if clinical_cat_df.empty:
        ...
        with right:
            plot_bar(show_display_df, "類別標記", "樣本數", f"{selected_feature} 樣本數", color="類別標記")
    ...
    if not clinical_num_df.empty:
        ...
        show_num_display_df = show_num_df.rename(columns={
            "feature": "欄位",
            "no_risk_mean": "無風險紀錄平均值（target=0）",
            "risk_mean": "風險紀錄平均值（target=1）",
            "difference": "平均差異",
...
# =========================================================
    ...
    st.title("流程管理｜使用者輸入、分流流程與管理者儀表板")
...
st.markdown(
    """
    <div class="footer">
    AI 智慧心血管疾病風險預測與健康管理系統｜六大模組展示版 v5.2.1｜健康風險評估原型，不取代醫師診斷
    </div>
    """,
    unsafe_allow_html=True,
)
