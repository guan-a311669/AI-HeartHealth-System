...
# 給前端選單使用的中文說明，避免只看到 cp、fbs、thal 這種欄位代碼
FEATURE_MENU_LABELS = {
    "sex": "sex｜性別（0=女性，1=男性）",
    "cp": "cp｜胸痛型態（1=典型心絞痛，2=非典型心絞痛，3=非心絞痛疼痛，4=無症狀）",
    "fbs": "fbs｜空腹血糖是否大於 120 mg/dl（0=否，1=是）",
    "restecg": "restecg｜靜息心電圖（0=正常，1=ST-T異常，2=左心室肥大可能）",
    "exang": "exang｜運動誘發心絞痛（0=否，1=是）",
    "slope": "slope｜ST 斜率（1=上升，2=平坦，3=下降）",
    "ca": "ca｜螢光顯影血管數（0~3）",
    "thal": "thal｜Thal 檢查（3=正常，6=固定缺陷，7=可逆缺陷）",
    "age": "age｜年齡",
    "trestbps": "trestbps｜靜息血壓",
    "chol": "chol｜膽固醇",
    "thalach": "thalach｜最大心率",
    "oldpeak": "oldpeak｜運動後 ST 變化數值",
}
...
# =========================================================
    ...
    with st.expander("看不懂欄位代碼？點這裡看指標對照"):
        quick_dict_df = pd.DataFrame([
            {"欄位代碼": "cp", "指標名稱": "胸痛型態", "畫面解讀": "不同胸痛型態在資料中的風險紀錄比例"},
            {"欄位代碼": "trestbps", "指標名稱": "靜息血壓", "畫面解讀": "血壓高低與風險紀錄的關係"},
            {"欄位代碼": "chol", "指標名稱": "膽固醇", "畫面解讀": "膽固醇高低與風險紀錄的關係"},
            {"欄位代碼": "fbs", "指標名稱": "空腹血糖", "畫面解讀": "空腹血糖是否大於 120 mg/dl"},
            {"欄位代碼": "restecg", "指標名稱": "靜息心電圖", "畫面解讀": "心電圖標記與風險紀錄的關係"},
            {"欄位代碼": "thalach", "指標名稱": "最大心率", "畫面解讀": "最大心率在兩組資料中的差異"},
            {"欄位代碼": "exang", "指標名稱": "運動誘發心絞痛", "畫面解讀": "運動時是否誘發心絞痛"},
            {"欄位代碼": "oldpeak", "指標名稱": "運動後 ST 變化數值", "畫面解讀": "心電圖相關數值變化"},
            {"欄位代碼": "slope", "指標名稱": "ST 斜率", "畫面解讀": "ST 斜率型態與風險紀錄的關係"},
            {"欄位代碼": "ca", "指標名稱": "螢光顯影血管數", "畫面解讀": "顯影血管數與風險紀錄的關係"},
            {"欄位代碼": "thal", "指標名稱": "Thal 檢查", "畫面解讀": "Thal 檢查分類與風險紀錄的關係"},
        ])
        st.dataframe(quick_dict_df, use_container_width=True)

    if clinical_cat_df.empty:
        st.warning("找不到 clinical_categorical_risk_table.csv。")
    else:
        feature_options = clinical_cat_df["feature"].dropna().unique().tolist()

        selected_feature = st.selectbox(
            "選擇要看的指標",
            feature_options,
            index=0,
            format_func=lambda x: FEATURE_MENU_LABELS.get(x, x),
        )

        selected_feature_title = FEATURE_SHORT_LABELS.get(selected_feature, selected_feature)

        st.caption(f"目前查看：{FEATURE_MENU_LABELS.get(selected_feature, selected_feature)}")
        ...
        if "欄位代碼" in show_display_df.columns:
            show_display_df.insert(
                1,
                "指標名稱",
                show_display_df["欄位代碼"].map(FEATURE_SHORT_LABELS).fillna(show_display_df["欄位代碼"]),
            )

        left, right = st.columns([1.15, 1])

        with left:
            plot_bar(show_display_df, "類別標記", "風險紀錄比例（%）", f"{selected_feature_title} 各類別風險紀錄比例", color="類別標記")

        with right:
            plot_bar(show_display_df, "類別標記", "資料筆數", f"{selected_feature_title} 資料筆數", color="類別標記")

        st.dataframe(show_display_df, use_container_width=True)

    st.markdown("### 數值指標比較：有風險紀錄 vs 無風險紀錄")
    if not clinical_num_df.empty:
        numeric_options = clinical_num_df["feature"].dropna().unique().tolist()

        selected_numeric_features = st.multiselect(
            "選擇要比較的數值指標",
            numeric_options,
            default=numeric_options,
            format_func=lambda x: NUMERIC_MENU_LABELS.get(x, x),
        )

        show_num_df = clinical_num_df[clinical_num_df["feature"].isin(selected_numeric_features)].copy()

        if show_num_df.empty:
            st.warning("請至少選擇一個數值指標。")
        else:
            show_num_df = show_num_df.sort_values("difference", ascending=False)
            show_num_plot_df = show_num_df.copy()
            show_num_plot_df["指標名稱"] = show_num_plot_df["feature"].map(FEATURE_SHORT_LABELS).fillna(show_num_plot_df["feature"])

            plot_bar(show_num_plot_df, "指標名稱", "difference", "兩組平均值差異", color="指標名稱")
            ...
            if "欄位代碼" in show_num_display_df.columns:
                show_num_display_df.insert(
                    1,
                    "指標名稱",
                    show_num_display_df["欄位代碼"].map(FEATURE_SHORT_LABELS).fillna(show_num_display_df["欄位代碼"]),
                )
